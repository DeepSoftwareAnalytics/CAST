...
from c2nl.models.transformer_serial import Transformer_Serial
from c2nl.models.transformer_RvNN_serial import Transformer_RvNN_Serial
from c2nl.models.ASTNN import ASTNN
from c2nl.models.CODEASTNN import CodeASTNN
from c2nl.models.transformer_parallel import Transformer_Parallel
from c2nl.models.transformer_flat import Transformer_Flat
...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, args, src_dict, tgt_dict, state_dict=None, ast_dict=None):
        # Book-keeping.
        self.args = args
        ...
        if ast_dict:
            self.args.ast_vocab_size = len(ast_dict)
        else:
            self.args.ast_vocab_size = 0
        if args.model_type == 'rnn':
            self.network = Seq2seq(self.args, tgt_dict)
        elif args.model_type == 'c2nl_transformer':
            self.network = Transformer(self.args, tgt_dict)
        elif args.model_type == 'x_serial':
            self.network = Transformer_Serial(self.args, tgt_dict)
        elif args.model_type == 'x_RvNN_serial':
            self.network = Transformer_RvNN_Serial(self.args, tgt_dict)
        elif args.model_type == 'x_parallel':
            self.network = Transformer_Parallel(self.args, tgt_dict)
        elif args.model_type == 'x_flat':
            self.network = Transformer_Flat(self.args, tgt_dict)
        elif args.model_type == 'x_hierarchical':
            pass

        elif args.model_type == 'astnn':
            self.network = ASTNN(self.args, tgt_dict)
        elif args.model_type == 'code_astnn':
            self.network = CodeASTNN(self.args, tgt_dict)
    ...
    def update(self, ex):
        ...
        ast_node_rep = ex['ast_node_rep']
        rebuild_tree_rep = ex['rebuild_tree_rep']
        ...
        if self.args.model_type == 'c2nl_transformer':
        ...
        elif self.args.model_type in ['x_serial', "x_parallel", 'x_flat', "code_astnn"]:
            net_loss = self.network(code_word_rep=code_word_rep,
                                    code_char_rep=code_char_rep,
                                    code_type_rep=code_type_rep,
                                    code_len=code_len,
                                    ast_node_rep=ast_node_rep,
                                    summ_word_rep=summ_word_rep,
                                    summ_char_rep=summ_char_rep,
                                    summ_len=summ_len,
                                    tgt_seq=tgt_seq,
                                    ...
                                    code_mask_rep=code_mask_rep,
                                    example_weights=ex_weights)
        elif self.args.model_type in ['x_RvNN_serial']:
            net_loss = self.network(code_word_rep=code_word_rep,
                                    code_char_rep=code_char_rep,
                                    code_type_rep=code_type_rep,
                                    code_len=code_len,
                                    ...
                                    code_mask_rep=code_mask_rep,
                                    example_weights=ex_weights,
                                    rebuild_tree_rep=rebuild_tree_rep)
        elif self.args.model_type in ['astnn']:
            net_loss = self.network(code_word_rep=code_word_rep,
                                    code_char_rep=code_char_rep,
                                    code_type_rep=code_type_rep,
                                    code_len=code_len,
        ...
        clip_grad_norm_(self.network.parameters(), self.args.clip)
        self.optimizer.step()
    ...
    def predict(self, ex, replace_unk=False):
        ...
        # self.network.cuda()
        self.network.eval()
        if self.args.use_asts:
            if self.args.is_DDP:
                self.network.module.astEncoder.batch_size = self.args.test_batch_size
            else:
                self.network.astEncoder.batch_size = self.args.test_batch_size
        source_map, alignment = None, None
        blank, fill = None, None
        # To enable copy attn, collect source map and alignment info
        if self.args.copy_attn:
        ...
        if self.args.use_asts:
            if self.args.is_DDP:
                self.network.module.astEncoder.batch_size = self.args.batch_size
            else:
                self.network.astEncoder.batch_size = self.args.batch_size
        # return predictions, targets, decoder_out['copy_info']
        return predictions, targets, decoder_out
    ...
    def load(filename, new_args=None):
        ...
        ast_dict = saved_params['ast_dict']
        state_dict = saved_params['state_dict']
        args = saved_params['args']
        if new_args:
            args = override_model_args(args, new_args)
        return Code2NaturalLanguage(args, src_dict, tgt_dict, state_dict=state_dict, ast_dict=ast_dict)
    ...
    def load_checkpoint(filename, use_gpu=True):
        ...
        args = saved_params['args']
        model = Code2NaturalLanguage(args, src_dict, tgt_dict, state_dict=state_dict, ast_dict=ast_dict)
    ...
    @staticmethod
    def load_checkpoint_load(filename_model,filename_chepoint, use_gpu=True):
        logger.info('Loading model %s' % filename_model)
        saved_params = torch.load(
            filename_model, map_location="cpu"
        )
        src_dict = saved_params['src_dict']
        tgt_dict = saved_params['tgt_dict']
        ast_dict = saved_params['ast_dict']
        ...
        model = Code2NaturalLanguage(args, src_dict, tgt_dict, state_dict=state_dict, ast_dict=ast_dict)
        del saved_params
        logger.info('Loading chekpoint model %s' % filename_chepoint)
        # saved_params = torch.load(
        #     filename, map_location=lambda storage, loc: storage
        # )
        saved_params1 = torch.load(filename_chepoint, map_location="cpu")
        epoch = saved_params1['epoch']
        # updates = saved_params1['updates']
        # optimizer = saved_params1['optimizer']
        # model.updates = updates
