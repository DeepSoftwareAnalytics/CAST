...
if __name__ == '__main__':
    ...
    if Config.is_multi_processing:
        ...
        for part in ["val", "train", "test"]:
            ...
            save_pickle_data(os.path.join(asts_path, part), "sliced_AST.pkl", asts_tree)
            del asts_tree

        asts_tree_train = read_pickle_data(os.path.join(asts_path, "train", "sliced_AST.pkl"))
        asts_tree_val = read_pickle_data(os.path.join(asts_path, "val", "sliced_AST.pkl"))
        asts_tree_test = read_pickle_data(os.path.join(asts_path, "test", "sliced_AST.pkl"))
    ...
    debug_logger(" time cost :" + time_format(time.perf_counter() - start))
    save_pickle_data(asts_path, 'sliced_AST.pkl', asts_tree)
