...
def vectorize(ex, model):
    ...
    code, summary, ast, rebuild_tree = ex['code'], ex['summary'], ex["ast"], ex["rebuild_tree"]
    vectorized_ex = dict()
    ...
    vectorized_ex['ast_node_rep'] = None
    if model.args.use_asts:
        vectorized_ex['ast_node_rep'] = ast[1]

    vectorized_ex['rebuild_tree_rep'] = None
    if model.args.is_rebuild_tree:
        vectorized_ex['rebuild_tree_rep'] = rebuild_tree[1]
    ...
    vectorized_ex['use_ast_word'] = model.args.use_ast_word
...
def batchify(batch):
    ...
    use_ast_word = batch[0]['use_ast_word']
    use_tgt_word = batch[0]['use_tgt_word']

    use_src_char = batch[0]['use_src_char']
    # use_ast_char = batch[0]['use_ast_char']
    ...
    # ----------Prepare ASTs tensor---------------
    ast_node_rep = None
    if batch[0]['ast_node_rep']:
        ast_node_rep = [ex['ast_node_rep'] for ex in batch]

    rebuild_tree_rep = None
    if batch[0]['rebuild_tree_rep']:
        rebuild_tree_rep = [ex['rebuild_tree_rep'] for ex in batch]
