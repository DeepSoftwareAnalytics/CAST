...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.code_context_attn = MultiHeadedAttention(
            heads, d_model, d_k, d_v, dropout=dropout,
            coverage=coverage_attn)
        self.ast_context_attn = MultiHeadedAttention(
            heads, d_model, d_k, d_v, dropout=dropout,
            coverage=coverage_attn)
    ...
    def forward(self,
        ...
        Args:
            ...
            subtree_emb (FloatTensor): ``(batch_size, subtree_number, model_dim)``
        Returns:
        ...
        # batch_size, subtree_number + src_len, model_dim
        cat_input_embedding = torch.cat((subtree_emb, memory_bank), 1)  # concatnate by len
        mid, attn, coverage = self.code_context_attn(cat_input_embedding,
                                                     cat_input_embedding,
                                                     query_norm,
                                                     mask=src_pad_mask,
                                                     layer_cache=layer_cache,
        ...
        # mid_norm = self.layer_norm_2(self.drop(mid_ast) + self.drop(mid) + query_norm)
        output = self.feed_forward(mid_norm)
        return output, attn, coverage


class FlatTransformerDecoder(DecoderBase):
    ...
    def __init__(self,
                 ...
                 max_relative_positions=0,
                 coverage_attn=False):
        super(FlatTransformerDecoder, self).__init__()
    ...
    def forward(self,
        ...
        code_pad_mask = ~sequence_mask(state["src_len"],
                                       max_len=state["src_max_len"]).unsqueeze(1)
        ast_len = [subtree_emb.shape[1]] * subtree_emb.shape[0]
        ast_len =torch.tensor(ast_len).to(state["src_len"].device)
        ast_pad_mask = ~sequence_mask(ast_len,
                                      subtree_emb.shape[1]).unsqueeze(1)
        src_pad_mask = torch.cat((ast_pad_mask, code_pad_mask), 2)
