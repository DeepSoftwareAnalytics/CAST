...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        mid_ast, ast_attn, _ = self.ast_context_attn(subtree_emb,
                                                     subtree_emb,
                                                     query_norm,
                                                     mask=None,
                                                     layer_cache=layer_cache,
        ...
        # mid_ast_norm = self.layer_norm_2(self.drop(mid_ast) + query_norm)

        mid, code_attn, coverage = self.code_context_attn(memory_bank,
                                                          memory_bank,
                                                          query_norm,
                                                          mask=src_pad_mask,
                                                          layer_cache=layer_cache,
                                                          ...
                                                          step=step,
                                                          coverage=coverage)
        # mid_norm_code = self.layer_norm_2(self.drop(mid) + query_norm)
        mid_norm = self.layer_norm_2(self.drop(mid_ast) + self.drop(mid) + query_norm)
        output = self.feed_forward(mid_norm)
        return output, ast_attn, code_attn, coverage


class ParallelTransformerDecoder(DecoderBase):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layer):
            ...
            output, ast_attn, code_attn, coverage = layer(
                output,
                mem_bank,
                src_pad_mask,
                tgt_pad_mask,
            ...
            std_attentions.append(code_attn)
    ...
    def _init_cache(self, state):
        ...
        for i, layer in enumerate(self.layer):
            ...
            layer_cache["ast_keys"] = None
            layer_cache["ast_values"] = None
