...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, layer_cache=None,
                attn_type=None, step=None, coverage=None, context_type="code"):
        """
            ...
            if attn_type == "self":
                ...
                if context_type == "code":
                    ...
                    if layer_cache["ast_keys"] is None:
                        ...
                        key, value = layer_cache["ast_keys"], \
                                     layer_cache["ast_values"]
                    layer_cache["ast_keys"] = key
                    layer_cache["ast_values"] = value
        else:
            key = shape(self.key(key), self.d_k)
            value = shape(self.value(value), self.d_v)
            query = shape(self.query(query), self.d_k)
