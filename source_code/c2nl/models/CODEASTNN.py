...
class CodeASTNN(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        # --------------- astnn ----------------
        word2vec = Word2Vec.load(args.w2v_path).wv
        max_tokens = word2vec.syn0.shape[0]  # 3000
        embedding_dim = word2vec.syn0.shape[1]
        embeddings = np.zeros((max_tokens + 1, embedding_dim), dtype="float32")
        embeddings[:word2vec.syn0.shape[0]] = word2vec.syn0
        self.astEncoder = BatchASTEncoder(args.node_embedding_dim, args.RvNN_input_dim,
                                       max_tokens + 1, args.RvNN_input_dim, args.batch_size,
                                                         args.use_gpu, pretrained_weight=embeddings)
        # --------------- astnn ----------------
        self.decoder = Decoder(args, self.embedder.dec_input_size)
    ...
    def _run_forward_ml(self,
        ...
        split_subtree_emb, split_full_tree_embedding = self.astEncoder(ast_node_rep)
