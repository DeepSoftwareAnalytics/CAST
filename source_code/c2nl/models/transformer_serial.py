...
#
#         self.code_enc_input_size = 0
#         self.dec_input_size = 0
#         self.ast_enc_input_size = 0  # AST node
#
...
#         assert args.use_ast_word or args.use_ast_char
#
#         self.use_src_word = args.use_src_word
#         self.use_tgt_word = args.use_tgt_word
#         self.use_ast_word = args.use_ast_word
#         if self.use_src_word:
#             self.src_word_embeddings = Embeddings(args.code_embedding_dim,
#                                                   args.src_vocab_size,
#                                                   constants.PAD)
#             self.code_enc_input_size += args.code_embedding_dim
#         if self.use_ast_word:
#             self.ast_word_embeddings = Embeddings(args.node_embedding_dim,
#                                                   args.ast_vocab_size,
#                                                   constants.PAD)
#             self.ast_enc_input_size += args.node_embedding_dim
#         if self.use_tgt_word:
#             self.tgt_word_embeddings = Embeddings(args.summary_embedding_dim,
#                                                   args.tgt_vocab_size,
#                                                   constants.PAD)
#             self.dec_input_size += args.summary_embedding_dim
#
#         self.use_src_char = args.use_src_char
#         self.use_ast_char = args.use_ast_char
...
#                                                      args.filter_size,
#                                                      args.nfilters)
#             self.code_enc_input_size += sum(list(map(int, args.nfilters)))
#             self.src_highway_net = Highway(self.code_enc_input_size, num_layers=2)
#
#         if self.use_ast_char:
#             assert len(args.filter_size) == len(args.nfilters)
#             self.ast_char_embeddings = CharEmbedding(args.n_characters,
#                                                      args.char_emsize,
#                                                      args.filter_size,
#                                                      args.nfilters)
#             self.ast_enc_input_size += sum(list(map(int, args.nfilters)))
#             self.ast_highway_net = Highway(self.ast_enc_input_size, num_layers=2)
#
...
#             #                                        self.enc_input_size)
#             self.src_pos_embeddings = nn.Embedding(args.code_max_len,
#                                                    self.code_enc_input_size)
#
#         if self.tgt_pos_emb:
#             # self.tgt_pos_embeddings = nn.Embedding(args.max_tgt_len + 2,
#             #                                        self.dec_input_size)
#             self.tgt_pos_embeddings = nn.Embedding(args.sum_max_len + 2,
#                                                    self.dec_input_size)
#
...
class CodeEncoder(nn.Module):
    def __init__(self,
        ...
        self.transformer = TransformerEncoder(num_layers=args.enc_layers,
                                              d_model=input_size,
                                              heads=args.nhead,
                                              d_k=args.d_k,
                                              d_v=args.d_v,
...
class Transformer_Serial(nn.Module):
    """Module that writes an answer for the question given a passage."""

    def __init__(self, args, tgt_dict):
        """"Constructor of the class."""
        super(Transformer_Serial, self).__init__()
        ...
        self.astEncoder = MultiTreeRvNNEncoder(args)
    ...
    def _run_forward_ml(self,
        ...
        #  [batch_size, subtree_num, RvNN_hidden_dim]
